function Flag() {
  return (
    <>
      <svg width="0" height="0">
        <symbol
          viewBox="0 0 512 512"
          id="flag-ar"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aaa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aaa)">
            <path fill="#496e2d" d="M0 0h512v512H0z"></path>
            <g fill="#eee">
              <path d="M144.7 306c0 18.5 15 33.5 33.4 33.5h100.2a27.8 27.8 0 0027.8 27.8h33.4a27.8 27.8 0 0027.8-27.8V306zm225.4-161.3v78c0 12.2-10 22.2-22.3 22.2v33.4c30.7 0 55.7-25 55.7-55.7v-77.9H370zm-239.3 78c0 12.2-10 22.2-22.3 22.2v33.4c30.7 0 55.7-25 55.7-55.7v-77.9h-33.4z"></path>
              <path d="M320 144.7h33.4v78H320zm-50 44.5a5.6 5.6 0 01-11.2 0v-44.5h-33.4v44.5a5.6 5.6 0 01-11.1 0v-44.5h-33.4v44.5a39 39 0 0039 39 38.7 38.7 0 0022.2-7 38.7 38.7 0 0022.2 7c1.7 0 3.4-.1 5-.3a22.3 22.3 0 01-21.6 17v33.4c30.6 0 55.6-25 55.6-55.7v-77.9H270z"></path>
              <path d="M180.9 244.9h50v33.4h-50z"></path>
            </g>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-bg"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aba">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aba)">
            <path
              fill="#496e2d"
              d="M0 166.9l258-31.7 254 31.7v178l-251.4 41.3L0 344.9z"
            ></path>
            <path fill="#eee" d="M0 0h512v166.9H0z"></path>
            <path fill="#d80027" d="M0 344.9h512V512H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-br"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aca">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aca)">
            <path fill="#6da544" d="M0 0h512v512H0z"></path>
            <path
              fill="#ffda44"
              d="M256 100.2L467.5 256 256 411.8 44.5 256z"
            ></path>
            <path
              fill="#eee"
              d="M174.2 221a87 87 0 00-7.2 36.3l162 49.8a88.5 88.5 0 0014.4-34c-40.6-65.3-119.7-80.3-169.1-52z"
            ></path>
            <path
              fill="#0052b4"
              d="M255.7 167a89 89 0 00-41.9 10.6 89 89 0 00-39.6 43.4 181.7 181.7 0 01169.1 52.2 89 89 0 00-9-59.4 89 89 0 00-78.6-46.8zM212 250.5a149 149 0 00-45 6.8 89 89 0 0010.5 40.9 89 89 0 00120.6 36.2 89 89 0 0030.7-27.3A151 151 0 00212 250.5z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-cn"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ada">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ada)">
            <path fill="#d80027" d="M0 0h512v512H0z"></path>
            <path
              fill="#ffda44"
              d="M140.1 155.8l22.1 68h71.5l-57.8 42.1 22.1 68-57.9-42-57.9 42 22.2-68-57.9-42.1H118zm163.4 240.7l-16.9-20.8-25 9.7 14.5-22.5-16.9-20.9 25.9 6.9 14.6-22.5 1.4 26.8 26 6.9-25.1 9.6zm33.6-61l8-25.6-21.9-15.5 26.8-.4 7.9-25.6 8.7 25.4 26.8-.3-21.5 16 8.6 25.4-21.9-15.5zm45.3-147.6L370.6 212l19.2 18.7-26.5-3.8-11.8 24-4.6-26.4-26.6-3.8 23.8-12.5-4.6-26.5 19.2 18.7zm-78.2-73l-2 26.7 24.9 10.1-26.1 6.4-1.9 26.8-14.1-22.8-26.1 6.4 17.3-20.5-14.2-22.7 24.9 10.1z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-cz"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aea">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aea)">
            <path fill="#eee" d="M0 0h512v256l-265 45.2z"></path>
            <path fill="#d80027" d="M210 256h302v256H0z"></path>
            <path fill="#0052b4" d="M0 0v512l256-256L0 0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-da"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="afa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#afa)">
            <path
              fill="#d80027"
              d="M0 0h133.6l32.7 20.3 34-20.3H512v222.6L491.4 256l20.6 33.4V512H200.3l-31.7-20.4-35 20.4H0V289.4l29.4-33L0 222.7z"
            ></path>
            <path
              fill="#eee"
              d="M133.6 0v222.6H0v66.8h133.6V512h66.7V289.4H512v-66.8H200.3V0h-66.7z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-de"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aga">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aga)">
            <path fill="#ffda44" d="M0 345l256.7-25.5L512 345v167H0z"></path>
            <path fill="#d80027" d="M0 167l255-23 257 23v178H0z"></path>
            <path fill="#333" d="M0 0h512v167H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-dk"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aha">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aha)">
            <path
              fill="#d80027"
              d="M0 0h133.6l32.7 20.3 34-20.3H512v222.6L491.4 256l20.6 33.4V512H200.3l-31.7-20.4-35 20.4H0V289.4l29.4-33L0 222.7z"
            ></path>
            <path
              fill="#eee"
              d="M133.6 0v222.6H0v66.8h133.6V512h66.7V289.4H512v-66.8H200.3V0h-66.7z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-ee"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aia">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aia)">
            <path
              fill="#333"
              d="M0 167l254.6-36.6L512 166.9v178l-254.6 36.4L0 344.9z"
            ></path>
            <path fill="#0052b4" d="M0 0h512v166.9H0z"></path>
            <path fill="#eee" d="M0 344.9h512V512H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-el"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aja">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aja)">
            <path
              fill="#eee"
              d="M0 122.5L89 0h67l356 55.7v66.7l-28.4 33.5 28.4 33.3V256l-26.6 32.2 26.6 34.6v66.8L485 421l27 35.3V512H0v-55.7l27.2-34.2L0 389.6v-66.8L30.6 251 0 189.2z"
            ></path>
            <path
              fill="#338af3"
              d="M0 0v122.5h89V0zm155.9 0v122.4H256V55.7h256V0H155.9zM256 122.4v66.8h256v-66.8zm0 66.8H155.9V256H89v-66.8H0v133.6h512V256H256zM0 389.6v66.7h512v-66.7z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-en"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aka">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aka)">
            <path
              fill="#eee"
              d="M0 256L256 0h256v55.7l-20.7 34.5 20.7 32.2v66.8l-21.2 32.7L512 256v66.8l-24 31.7 24 35.1v66.7l-259.1 28.3L0 456.3v-66.7l27.1-33.3L0 322.8z"
            ></path>
            <path
              fill="#d80027"
              d="M256 256h256v-66.8H236.9zm-19.1-133.6H512V55.7H236.9zM512 512v-55.7H0V512zM0 389.6h512v-66.8H0z"
            ></path>
            <path fill="#0052b4" d="M0 0h256v256H0z"></path>
            <path
              fill="#eee"
              d="M15 14.5L6.9 40H-20L1.7 55.8l-8.3 25.5L15 65.5l21.6 15.8-8.2-25.4L50.2 40H23.4zm91.8 0L98.5 40H71.7l21.7 15.8-8.3 25.5 21.7-15.8 21.7 15.8-8.3-25.4L142 40h-26.8zm91.9 0l-8.3 25.6h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16H207zM15 89.2l-8.3 25.5H-20l21.7 15.8-8.3 25.5L15 140l21.6 15.7-8.2-25.3 21.7-16H23.4zm91.8 0l-8.3 25.5H71.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16h-26.8zm91.8 0l-8.3 25.5h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16H207zM15 163.6l-8.3 25.5H-20L1.6 205l-8.3 25.5L15 214.6l21.7 15.8-8.3-25.4 21.7-15.9H23.3zm91.8 0l-8.3 25.5H71.7L93.4 205l-8.3 25.5 21.7-15.8 21.7 15.8-8.3-25.4 21.7-15.9h-26.8zm91.8 0l-8.3 25.5h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.7 15.8L212 205l21.7-15.9H207z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-es"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ala">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ala)">
            <path
              fill="#d80027"
              d="M0 0h512v128l-39.8 130.3L512 384v128H0V384l37.8-124L0 128z"
            ></path>
            <path fill="#ffda44" d="M0 128h512v256H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-et"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ama">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ama)">
            <path
              fill="#333"
              d="M0 167l254.6-36.6L512 166.9v178l-254.6 36.4L0 344.9z"
            ></path>
            <path fill="#0052b4" d="M0 0h512v166.9H0z"></path>
            <path fill="#eee" d="M0 344.9h512V512H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-fi"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ana">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ana)">
            <path
              fill="#eee"
              d="M0 0h133.6l35.3 16.7L200.3 0H512v222.6l-22.6 31.7 22.6 35.1V512H200.3l-32-19.8-34.7 19.8H0V289.4l22.1-33.3L0 222.6z"
            ></path>
            <path
              fill="#0052b4"
              d="M133.6 0v222.6H0v66.8h133.6V512h66.7V289.4H512v-66.8H200.3V0h-66.7z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-fil"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aoa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aoa)">
            <path fill="#0052b4" d="M0 0h512v256l-265 45.2z"></path>
            <path fill="#d80027" d="M210 256h302v256H0z"></path>
            <path fill="#eee" d="M0 0v512l256-256z"></path>
            <path
              fill="#ffda44"
              d="M175.3 256L144 241.3l16.7-30.3-34 6.5-4.3-34.3-23.6 25.2L75 183.2l-4.3 34.3-34-6.5 16.7 30.3L22.3 256l31.2 14.7L37 301l34-6.5 4.2 34.3 23.7-25.2 23.6 25.2 4.3-34.3 34 6.5-16.7-30.3zm-107-155.8l10.4 14.5 17-5.4-10.6 14.4 10.4 14.5-17-5.6L68 147l.2-17.9-17-5.6 17-5.4zm0 264.8l10.4 14.6 17-5.4-10.6 14.3 10.4 14.6-17-5.7L68 411.8l.2-17.9-17-5.6 17-5.4zm148.4-132.4L206.3 247l-17-5.4 10.5 14.4-10.4 14.6 17-5.7 10.6 14.4-.1-17.9 17-5.6-17.1-5.4z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-fr"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="apa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#apa)">
            <path
              fill="#eee"
              d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"
            ></path>
            <path fill="#0052b4" d="M0 0h167v512H0z"></path>
            <path fill="#d80027" d="M345 0h167v512H345z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-hr"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aqa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aqa)">
            <path
              fill="#eee"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#d80027" d="M0 0h512v167H0z"></path>
            <path fill="#0052b4" d="M0 345h512v167H0z"></path>
            <path
              fill="#338af3"
              d="M322.8 178h-44.5l7.4-55.7 29.7-22.2 29.6 22.2V167zm-133.6 0h44.5l-7.4-55.7-29.7-22.2-29.6 22.2V167z"
            ></path>
            <path
              fill="#0052b4"
              d="M285.7 178h-59.4v-55.7l29.7-22.2 29.7 22.2z"
            ></path>
            <path
              fill="#eee"
              d="M167 167v122.3a89 89 0 0035.8 71.3l15.5-3.9 19.7 19.8a89.1 89.1 0 0018 1.8 89 89 0 0017.9-1.8l22.4-18.7 13 2.8a89 89 0 0035.7-71.3V167z"
            ></path>
            <path
              fill="#d80027"
              d="M167 167h35.6v35.5H167zm71.2 0h35.6v35.5h-35.6zm71.2 0H345v35.5h-35.6zm-106.8 35.5h35.6v35.6h-35.6zm71.2 0h35.6v35.6h-35.6zM167 238.1h35.6v35.6H167zm35.6 35.6h35.6v35.6h-35.6zm35.6-35.6h35.6v35.6h-35.6zm71.2 0H345v35.6h-35.6zm-35.6 35.6h35.6v35.6h-35.6zm-35.6 35.6h35.6V345h-35.6zm-35.6 0h-33.3c3 13.3 9 25.4 17.3 35.6h16zM309.4 345h16a88.8 88.8 0 0017.3-35.6h-33.3zm-106.8 0v15.6a88.7 88.7 0 0035.6 16V345zm71.2 0v31.6a88.7 88.7 0 0035.6-16V345z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-hu"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ara">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ara)">
            <path
              fill="#eee"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#d80027" d="M0 0h512v167H0z"></path>
            <path fill="#6da544" d="M0 345h512v167H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-it"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="asa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#asa)">
            <path
              fill="#eee"
              d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"
            ></path>
            <path fill="#6da544" d="M0 0h167v512H0z"></path>
            <path fill="#d80027" d="M345 0h167v512H345z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-ja"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ata">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ata)">
            <path fill="#eee" d="M0 0h512v512H0z"></path>
            <circle cx="256" cy="256" r="111.3" fill="#d80027"></circle>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-lt"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aua">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aua)">
            <path
              fill="#6da544"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#ffda44" d="M0 0h512v167H0z"></path>
            <path fill="#d80027" d="M0 345h512v167H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-lv"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="ava">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#ava)">
            <path
              fill="#a2001d"
              d="M0 0h512v189.2l-38.5 70 38.5 63.6V512H0V322.8l39.4-63L0 189.1z"
            ></path>
            <path fill="#eee" d="M0 189.2h512v133.6H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-nl"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="awa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#awa)">
            <path
              fill="#eee"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#a2001d" d="M0 0h512v167H0z"></path>
            <path fill="#0052b4" d="M0 345h512v167H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-no"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="axa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#axa)">
            <path
              fill="#d80027"
              d="M0 0h100.2l66.1 53.5L233.7 0H512v189.3L466.3 257l45.7 65.8V512H233.7l-68-50.7-65.5 50.7H0V322.8l51.4-68.5-51.4-65z"
            ></path>
            <path
              fill="#eee"
              d="M100.2 0v189.3H0v33.4l24.6 33L0 289.5v33.4h100.2V512h33.4l30.6-26.3 36.1 26.3h33.4V322.8H512v-33.4l-24.6-33.7 24.6-33v-33.4H233.7V0h-33.4l-33.8 25.3L133.6 0z"
            ></path>
            <path
              fill="#0052b4"
              d="M133.6 0v222.7H0v66.7h133.6V512h66.7V289.4H512v-66.7H200.3V0z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-ph"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aya">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aya)">
            <path fill="#0052b4" d="M0 0h512v256l-265 45.2z"></path>
            <path fill="#d80027" d="M210 256h302v256H0z"></path>
            <path fill="#eee" d="M0 0v512l256-256z"></path>
            <path
              fill="#ffda44"
              d="M175.3 256L144 241.3l16.7-30.3-34 6.5-4.3-34.3-23.6 25.2L75 183.2l-4.3 34.3-34-6.5 16.7 30.3L22.3 256l31.2 14.7L37 301l34-6.5 4.2 34.3 23.7-25.2 23.6 25.2 4.3-34.3 34 6.5-16.7-30.3zm-107-155.8l10.4 14.5 17-5.4-10.6 14.4 10.4 14.5-17-5.6L68 147l.2-17.9-17-5.6 17-5.4zm0 264.8l10.4 14.6 17-5.4-10.6 14.3 10.4 14.6-17-5.7L68 411.8l.2-17.9-17-5.6 17-5.4zm148.4-132.4L206.3 247l-17-5.4 10.5 14.4-10.4 14.6 17-5.7 10.6 14.4-.1-17.9 17-5.6-17.1-5.4z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-pl"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="aza">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#aza)">
            <path fill="#d80027" d="M0 256l256.4-44.3L512 256v256H0z"></path>
            <path fill="#eee" d="M0 0h512v256H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-ro"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="baa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#baa)">
            <path
              fill="#ffda44"
              d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"
            ></path>
            <path fill="#0052b4" d="M0 0h167v512H0z"></path>
            <path fill="#d80027" d="M345 0h167v512H345z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-ru"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bba">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bba)">
            <path
              fill="#0052b4"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#eee" d="M0 0h512v167H0z"></path>
            <path fill="#d80027" d="M0 345h512v167H0z"></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-se"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bca">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bca)">
            <path
              fill="#0052b4"
              d="M0 0h133.6l35.3 16.7L200.3 0H512v222.6l-22.6 31.7 22.6 35.1V512H200.3l-32-19.8-34.7 19.8H0V289.4l22.1-33.3L0 222.6z"
            ></path>
            <path
              fill="#ffda44"
              d="M133.6 0v222.6H0v66.8h133.6V512h66.7V289.4H512v-66.8H200.3V0z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-si"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bda">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bda)">
            <path
              fill="#0052b4"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#eee" d="M0 0h512v167H0z"></path>
            <path fill="#d80027" d="M0 345h512v167H0z"></path>
            <path fill="#0052b4" d="M222.7 167v-66.8H89V167l67 82.6z"></path>
            <path
              fill="#eee"
              d="M89 167v22.2c0 51.1 66.8 66.8 66.8 66.8s66.8-15.7 66.8-66.8V167l-22.3 22.2-44.5-33.4-44.5 33.4z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-sk"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bea">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bea)">
            <path
              fill="#0052b4"
              d="M0 167l253.8-19.3L512 167v178l-254.9 32.3L0 345z"
            ></path>
            <path fill="#eee" d="M0 0h512v167H0z"></path>
            <path fill="#d80027" d="M0 345h512v167H0z"></path>
            <path
              fill="#eee"
              d="M66.2 144.7v127.7c0 72.6 94.9 95 94.9 95s94.9-22.4 94.9-95V144.7z"
            ></path>
            <path
              fill="#d80027"
              d="M88.5 144.7v127.7a54 54 0 005.6 24.1h134a53.6 53.6 0 005.6-24.1V144.7z"
            ></path>
            <path
              fill="#eee"
              d="M205.6 233.7h-33.4v-22.2h22.3v-22.3h-22.3V167H150v22.2h-22.3v22.3H150v22.2h-33.4V256H150v22.3h22.2V256h33.4z"
            ></path>
            <path
              fill="#0052b4"
              d="M124.5 327.6a160 160 0 0036.6 16.6c7.5-2.3 22.3-7.6 36.6-16.6a80 80 0 0030.4-31 38.8 38.8 0 00-22.5-7.2c-3 0-6 .4-8.8 1a39 39 0 00-71.4 0 39 39 0 00-8.8-1c-8.4 0-16.1 2.6-22.5 7.1a80 80 0 0030.4 31.1z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-tl"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bfa">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bfa)">
            <path fill="#0052b4" d="M0 0h512v256l-265 45.2z"></path>
            <path fill="#d80027" d="M210 256h302v256H0z"></path>
            <path fill="#eee" d="M0 0v512l256-256z"></path>
            <path
              fill="#ffda44"
              d="M175.3 256L144 241.3l16.7-30.3-34 6.5-4.3-34.3-23.6 25.2L75 183.2l-4.3 34.3-34-6.5 16.7 30.3L22.3 256l31.2 14.7L37 301l34-6.5 4.2 34.3 23.7-25.2 23.6 25.2 4.3-34.3 34 6.5-16.7-30.3zm-107-155.8l10.4 14.5 17-5.4-10.6 14.4 10.4 14.5-17-5.6L68 147l.2-17.9-17-5.6 17-5.4zm0 264.8l10.4 14.6 17-5.4-10.6 14.3 10.4 14.6-17-5.7L68 411.8l.2-17.9-17-5.6 17-5.4zm148.4-132.4L206.3 247l-17-5.4 10.5 14.4-10.4 14.6 17-5.7 10.6 14.4-.1-17.9 17-5.6-17.1-5.4z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-us"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bga">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bga)">
            <path
              fill="#eee"
              d="M0 256L256 0h256v55.7l-20.7 34.5 20.7 32.2v66.8l-21.2 32.7L512 256v66.8l-24 31.7 24 35.1v66.7l-259.1 28.3L0 456.3v-66.7l27.1-33.3L0 322.8z"
            ></path>
            <path
              fill="#d80027"
              d="M256 256h256v-66.8H236.9zm-19.1-133.6H512V55.7H236.9zM512 512v-55.7H0V512zM0 389.6h512v-66.8H0z"
            ></path>
            <path fill="#0052b4" d="M0 0h256v256H0z"></path>
            <path
              fill="#eee"
              d="M15 14.5L6.9 40H-20L1.7 55.8l-8.3 25.5L15 65.5l21.6 15.8-8.2-25.4L50.2 40H23.4zm91.8 0L98.5 40H71.7l21.7 15.8-8.3 25.5 21.7-15.8 21.7 15.8-8.3-25.4L142 40h-26.8zm91.9 0l-8.3 25.6h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16H207zM15 89.2l-8.3 25.5H-20l21.7 15.8-8.3 25.5L15 140l21.6 15.7-8.2-25.3 21.7-16H23.4zm91.8 0l-8.3 25.5H71.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16h-26.8zm91.8 0l-8.3 25.5h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.6 15.7-8.2-25.3 21.7-16H207zM15 163.6l-8.3 25.5H-20L1.6 205l-8.3 25.5L15 214.6l21.7 15.8-8.3-25.4 21.7-15.9H23.3zm91.8 0l-8.3 25.5H71.7L93.4 205l-8.3 25.5 21.7-15.8 21.7 15.8-8.3-25.4 21.7-15.9h-26.8zm91.8 0l-8.3 25.5h-26.8l21.7 15.8-8.3 25.5 21.7-15.8 21.7 15.8L212 205l21.7-15.9H207z"
            ></path>
          </g>
        </symbol>
        <symbol
          viewBox="0 0 512 512"
          id="flag-vn"
          xmlns="http://www.w3.org/2000/svg"
        >
          <mask id="bha">
            <circle cx="256" cy="256" r="256" fill="#fff"></circle>
          </mask>
          <g mask="url(#bha)">
            <path fill="#d80027" d="M0 0h512v512H0z"></path>
            <path
              fill="#ffda44"
              d="M256 133.6l27.6 85H373L300.7 271l27.6 85-72.3-52.5-72.3 52.6 27.6-85-72.3-52.6h89.4z"
            ></path>
          </g>
        </symbol>
      </svg>
    </>
  )
}
export default Flag
